# Ревью кода-1 🌶🌶
# На обработку поступает последовательность из целых чисел.
...
# Примечание 1. Число не превышает по абсолютной величине 10^06, если -10^06 <= x <= 10^06.
# Примечание 2. При необходимости вы можете добавить необходимые строки кода.

count = 0
p = 1                               # p = 0
for i in range(1, 11):              # for i in range(1, 10)
    x = int(input())
    if x >= 0:                      # if x > 0
        p *= x                      # p = p * x
